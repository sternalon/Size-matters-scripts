...
def produce_figure_row(fig,axes,nrows,ncols,Delta_list, title_list,colorbar_unit_list,y_title_list,datapath, file_extension,cscale, row_num, \
		difference_on,pole,field_name,axes_direction='xy',significant_values_only=False):
			title=None
			for k in range(ncols):
				...
				if significant_values_only==False:
					p_values=None
				...
				if k==0 and row_num<3:
					...
					ax.annotate('Zonal Velocity', xy=(-0.45, 0.5), xycoords='axes fraction', fontsize=21,rotation=90,verticalalignment='center')
...
def main():
	...
	plot_SSU_fields=True
	plot_depth_int_u_fields=True
	plot_AB_plus_Ross_Zonal_Velocity_Section=True
	plot_EastA_plus_Adele_Zonal_Velocity_Section=True
	plot_Weddell_Zonal_Velocity_Section=True
	...
	#y_title_list=np.array(['Sea Surface Zonal_Velocity', 'AB+Ross', 'East Antarctia', 'Weddell','Depth Integrated Zonal_Velocity'])
	y_title_list=np.array(['Sea Surface','Column', 'AB+Ross', 'East Antarctia', 'Weddell'])
	#y_title_list=np.array(['SSU','Column Zonal_Velocity', 'AB+Ross', 'East Antarctia', 'Weddell'])
	colorbar_unit_list=np.array(['SSU (m/s)', '(m^2/s)','zonal velocity (m/s)', 'zonal velocity (m/s)','zonal velocity (m/s)'])
	nrows=5
	...
	if plot_all_panels_without_data==True:
		...
		#Plotting Melt
		if plot_SSU_fields==True:
			cscale=None
			#cscale=velocity_scale
			row_num=1
			difference_on=1
			axes_direction='xy'
			field_name='Sea Surface Zonal Velocity'
			file_extension='_UO_1959_to_2019_all_south_xy_anomaly.mat'
		...
		if plot_depth_int_u_fields==True:
			cscale=5
			row_num=2
			difference_on=1
			axes_direction='xy'
			field_name='Depth Integrated Zonal Velocity'
			file_extension='_u_1959_to_2019_all_south_xy_anomaly.mat'
			produce_figure_row(fig,axes,nrows,ncols,Delta_list, title_list,colorbar_unit_list,y_title_list, datapath, file_extension,cscale, \
					row_num,difference_on, pole,field_name,axes_direction,significant_vals_only)

		#Plotting AB_plus_Ross Zonal_Velocity
		if plot_AB_plus_Ross_Zonal_Velocity_Section==True:
			cscale=velocity_scale
			row_num=3
			difference_on=1
			axes_direction='yz'
			field_name='AB_plus_Ross Zonal_Velocity Section'
			file_extension='_u_1959_to_2019_all_south_yz_anomaly_AB_plus_Ross.mat'
			produce_figure_row(fig,axes,nrows,ncols,Delta_list, title_list,colorbar_unit_list,y_title_list, datapath, file_extension,cscale, \
					row_num,difference_on, pole,field_name,axes_direction,significant_vals_only)

		#Plotting EastA_plus_Adele Zonal_Velocity
		if plot_EastA_plus_Adele_Zonal_Velocity_Section==True:
			cscale=velocity_scale
			row_num=4
			difference_on=1
			axes_direction='yz'
			field_name='EastA_plus_Adele Zonal_Velocity Section'
			file_extension='_u_1959_to_2019_all_south_yz_anomaly_EastA_plus_AdeleLand.mat'
			produce_figure_row(fig,axes,nrows,ncols,Delta_list, title_list,colorbar_unit_list,y_title_list, datapath, file_extension,cscale, \
					row_num,difference_on, pole,field_name,axes_direction,significant_vals_only)
		#Plotting Weddell Zonal_Velocity
		if plot_Weddell_Zonal_Velocity_Section==True:
			cscale=velocity_scale
			row_num=5
			difference_on=1
			axes_direction='yz'
			field_name='Weddell Zonal_Velocity Section'
			file_extension='_u_1959_to_2019_all_south_yz_anomaly_Weddell.mat'
			produce_figure_row(fig,axes,nrows,ncols,Delta_list, title_list,colorbar_unit_list,y_title_list, datapath, file_extension,cscale, \
					row_num,difference_on, pole,field_name,axes_direction,significant_vals_only)
	...
	if save_figure==True:
		plt.savefig('paper_figures/Fig_U0_sections_u_D1_D3_D9.png')
	plt.show()

...
def produce_figure_row(fig,axes,nrows,ncols,Delta_list, title_list,colorbar_unit_list,y_title_list,datapath, file_extension,cscale, row_num, \
		difference_on,pole,field_name,axes_direction='xy',significant_vals_only):
			title=None
			...
			cbar_ax = fig.add_axes([0.85, 0.13+((nrows-row_num)*0.45), 0.03, 0.3])
...
def main():
	...
	y_title_list=np.array(['Sea Surface','Column Integrated', 'AB+Ross', 'East Antarctia', 'Weddell'])
	...
	if plot_all_panels_without_data==True:
		...
		if plot_depth_int_rho_fields==True:
			...
			produce_figure_row(fig,axes,nrows,ncols,Delta_list, title_list,colorbar_unit_list,y_title_list, datapath, file_extension,cscale, \
					row_num,difference_on, pole,field_name,significant_vals_only)
	...
	if save_figure==True:
		plt.savefig('paper_figures/Fig_SSD_depth_av_D1_D3_D9.png')
	plt.show()

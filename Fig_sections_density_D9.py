...
def main():
	...
	colorbar_unit_list=np.array(['density anomaly (kg/m$^{3}$)', 'density anomaly (kg/m$^{3}$)','density anomaly (kg/m$^{3}$)'])
	nrows=1
	...
	if plot_all_panels_without_data==True:
		...
		if plot_AB_plus_Ross_Density_Section==True:
			...
			file_extension='_rho_1959_to_2019_all_south_yz_anomaly_'+ Sector_list[plot_num-1]  +'.mat'
	...
	if save_figure==True:
		plt.savefig('paper_figures/Fig_sections_density_D9.png',dpi=300,bbox_inches='tight')
	plt.show()

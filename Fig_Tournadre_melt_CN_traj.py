...
def produce_figure_panel(fig,axes,nrows,ncols,Delta_list, title_list,colorbar_unit_list,y_title_list,datapath, file_extension,cscale, panel_num, \
		difference_on,pole,field_name,field_type_list,axes_direction='xy',significant_only=False,boundinglat=-45.):
			...
			if panel_num>1:
				...
				cbar=fig.colorbar(datamap, orientation="horizontal",pad=0.05,ticks=ticks)
			...
			text(1,1,letter_labels[panel_num-1], ha='right', va='bottom',transform=ax.transAxes,fontsize=15)
...
def main():
	...
	colorbar_unit_list=np.array(['SST (C))', 'melt (m/year)','Sea Ice Conc anomaly ', 'density (kg/$m^{3}$)','density (kg/$m^{3}$)'])
	nrows=1
	ncols=3
	...
	if plot_all_panels_without_data==True:
		...
		if plot_traj_fields==True:
			...
			cNorm = mpl.colors.LogNorm(vmin=8.8e7, vmax=1.e12)
			datamap=m.scatter(Total_berg_lon,Total_berg_lat, c=mass0, marker='o',cmap='jet',norm=cNorm, lw = 0.0)

			cbar=fig.colorbar(datamap, orientation="horizontal",pad=0.05)
			cbar.ax.tick_params(labelsize=20)
			cbar.set_label('Calving Mass (kg)',fontsize=20)
		...
		if plot_melt_fields==True:
			...
			produce_figure_panel(fig,axes,nrows,ncols,Delta_list, title_list,colorbar_unit_list,y_title_list, datapath, file_extension,cscale, \
					panel_num,difference_on, pole,field_name,field_type_list,axes_direction,significant_only=False,boundinglat=boundinglat)
		...
		if plot_CN_fields==True:
			...
			produce_figure_panel(fig,axes,nrows,ncols,Delta_list, title_list,colorbar_unit_list,y_title_list, datapath, file_extension,cscale, \
					panel_num,difference_on, pole,field_name,field_type_list,axes_direction,significant_only,boundinglat)
	...
	if save_figure==True:
		plt.savefig('paper_figures/Fig_Tournadre_melt_CN_traj.png',dpi=300,bbox_inches='tight')
	plt.show()

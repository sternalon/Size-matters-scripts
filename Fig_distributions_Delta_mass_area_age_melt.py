...
def plot_power_laws_on_figure(plot_1_2_line, plot_2_3_line, plot_3_2_line,x):
	if plot_1_2_line==True:
		y=x**(-0.5)
		#total=1e5
		total=10.**6.1
		y=y/total
		plt.plot(x,y, linewidth=1.5, linestyle="--",color='black',label='$y=x^{-1/2}$')
	...
	if plot_2_3_line==True:
		...
		total=10**(4.8)
		y=y/total
		plt.plot(x,y, linewidth=1.5, linestyle=":",color='black',label='$y=x^{-2/3}$')
...
def produce_delta_distribution_panel(datapath,nrows,ncols,position,field_name,Delta_list,Area_list,color_vec,letter_labels,mass_vs_area):
	...
	#for num in range(10):
		...
		ax=subplot(nrows,ncols,position)
		label=Delta_list[num]
		plt.plot(x,prob_dist, color=color_vec[num], linewidth=4.,label=label)
	...
	plot_1_2_line=False ;plot_2_3_line=False; plot_3_2_line=False
	if mass_vs_area=='mass':
		plot_1_2_line=True ;plot_2_3_line=True; plot_3_2_line=False

	plot_power_laws_on_figure(plot_1_2_line , plot_2_3_line ,plot_3_2_line,x)
...
def main():
	...
	plot_split_Tournadre_dist=False
	plot_split_Tournadre_dist_depth=False
	plot_Delta_distributions_area=True
	plot_Delta_distributions_mass=True
	plot_mass_vs_age=True
	...
	if plot_Delta_distributions_area==True:
		position=3
	...
	if plot_Delta_distributions_mass==True:
		position=4
		field_name='size distribution for Deltas'
		mass_vs_area='mass'
		produce_delta_distribution_panel(datapath,nrows,ncols,position,field_name,Delta_list,Area_list,color_vec,letter_labels,mass_vs_area)
	...
	if save_figure==True:
		plt.savefig('paper_figures/Fig_distributions_Delta_mass_area_age.png')
	plt.show()
